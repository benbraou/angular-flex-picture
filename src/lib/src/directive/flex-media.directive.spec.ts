...
@Component({
  template: `
<picture>
  <source [flexMedia]="'${TEST_BREAKPOINTS[0]
    .alias}'" srcset="https://dummyimage.com/600x400/c236c2/434de0.png">
  <source [flexMedia]="'${TEST_BREAKPOINTS[1]
    .alias}'" srcset="https://dummyimage.com/800x600/367ac2/434de0.png">
...
describe('Directive: FlexMedia', () => {
  ...
  it(
    'should set the media attribute with the media query matching the breakpoint alias',
    async(() => {
      ...
      expect(sourceElts[0].getAttribute('media')).toEqual(
        TEST_BREAKPOINTS[0].mediaQuery,
      );
      expect(sourceElts[1].getAttribute('media')).toEqual(
        TEST_BREAKPOINTS[1].mediaQuery,
      );
    }),
  );
});
