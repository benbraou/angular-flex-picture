...
var allSpecFiles =
    Object.keys(window.__karma__.files).filter(isSpecFile).filter(isBuiltFile);
...
System.config({
  ...
  packages: {
    ...
    '': {defaultExtension: 'js'},
    src: {defaultExtension: 'js', meta: {'./*.js': {loader: 'system-loader'}}}
  },
...
initTesting() {
  return Promise
      .all(allSpecFiles.map(function(moduleName) {
